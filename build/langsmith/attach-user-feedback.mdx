...
By default, feedback logged through the SDK extends a base-retention trace to extended retention; pass `extend_trace_retention=False` in Python or `extendTraceRetention: false` in TypeScript to leave the trace at its current retention tier.
