...
description: Reference for the failure categories LangSmith Engine assigns to detected issues, including descriptions, examples, and detection methods.
---

When [LangSmith Engine](/langsmith/engine) detects a recurring issue in your traces, it tags the issue with a category. This page lists every category Engine assigns, with a description, concrete example, and detection method for each. Engine automatically scans your traces and assigns the best-fitting category to each detected issue; you do not select categories manually.

The 16 categories below cover the most common agent failure patterns Engine has observed. When Engine finds a recurring issue that does not fit any of these categories cleanly, it assigns a short descriptive category name instead, so project-specific issues are still labeled and tracked.

## How Engine detects issues

Engine uses two detection methods, depending on the category:

- **Code evaluator**: Engine generates a deterministic code rule that checks for structural patterns in trace data. For example, a code evaluator can detect response truncation by checking whether the final output ends with a clean terminator, or detect agent looping by measuring the fraction of repeated tool calls. Engine specializes each code evaluator to your project's specific tool names, field paths, and observed patterns, so the exact rule may differ between projects even for the same category. Code evaluators produce consistent results across runs.
- **LLM judgment**: Engine uses LLM-based analysis to identify semantic patterns that cannot be checked with a code rule. For example, determining whether a response contains hallucinated facts requires comparing the agent's claims against tool outputs and reasoning about whether the facts are supported.

| Detection method | Categories |
| --- | --- |
| [Code evaluator](#categories-detected-with-code-evaluators) | [Agent looping](#agent-looping), [Context explosion](#context-explosion), [Failed error recovery](#failed-error-recovery), [Incorrect tool args](#incorrect-tool-args), [PII leak](#pii-leak), [Response truncation](#response-truncation), [Silent tool error](#silent-tool-error), [Tracing quality](#tracing-quality), [Wrong tool](#wrong-tool) |
| [LLM judgment](#categories-detected-with-llm-judgment) | [Feature gap](#feature-gap), [Flawed plan](#flawed-plan), [Guardrail bypass](#guardrail-bypass), [Hallucination](#hallucination), [Missing capability awareness](#missing-capability-awareness), [System prompt drift](#system-prompt-drift), [Task evasion](#task-evasion) |

## Categories detected with code evaluators
...
## Categories detected with LLM judgment
...
The 16 categories are defined in Engine's detection logic and represent the most common agent failure patterns Engine has observed.

The taxonomy is not exhaustive. Engine can assign additional descriptive category names for project-specific failure patterns that do not fit the predefined categories. If your project surfaces an issue with a category name not listed on this page, Engine identified a pattern specific to your application.

If Engine assigns a category that does not match the actual problem, you can mark the issue as ignored with a reason. Engine uses this feedback to improve its future analysis. For more information, see [Resolve or ignore an issue](/langsmith/engine#resolve-or-ignore-an-issue).

<Note>
LangSmith does not currently send notifications when the issue taxonomy changes. To stay informed, watch the [LangSmith Cloud changelog](/langsmith/changelog) or contact LangSmith support.
</Note>
