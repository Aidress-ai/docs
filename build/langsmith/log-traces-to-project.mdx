...
**How run IDs work across replicas:**

- The **primary** replica keeps the original run ID unchanged.
- Each **secondary** replica receives a deterministic run ID derived from the original run ID and the secondary replica's project name.
- Use `compute_run_id_for_secondary_replica(original_run_id, project_name)` to compute the secondary run ID and pass it when calling `create_feedback`.
...
import { computeRunIdForSecondaryReplica } from "langsmith/utils";
...
<Note>
The `compute_run_id_for_secondary_replica` / `computeRunIdForSecondaryReplica` helper is available in Python SDK >= 0.10.8 and JS SDK >= 0.8.5. If you are using an older SDK version, upgrade to use this feature.
</Note>
