...
- **`memories/**`**: durable long-term memory. The runtime remounts a scoped slice as `/memories/` (hot `/memories/AGENTS.md` plus optional cold files).
- **`org-memory/**`** (optional): org-wide facts mounted read-only at `/org-memory`.

Edit instructions and skills in your project and redeploy. Memory is runtime-owned, so deploy preserves `memories/**` instead of overwriting it. For hot/cold tiers, identity remounts, and local `.mda/__contexthub__`, see [Memory](/langsmith/managed-deep-agents-memory).
