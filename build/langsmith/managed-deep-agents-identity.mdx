...
- **Memory** — which Context Hub `/memories` slice the run remounts
...
| `memory` | `actor`, `tenant`, `agent`, `none` | Which Context Hub slice remounts as `/memories` (see [Memory](/langsmith/managed-deep-agents-memory)) |
...
<CardGroup cols={2}>
  <Card title="Memory" icon="brain" href="/langsmith/managed-deep-agents-memory">
    Persist preferences in the scoped `/memories` slice.
  </Card>
