...
Managed Deep Agents gives every deployment durable long-term memory in [Context Hub](/langsmith/use-the-context-hub). The agent reads and updates files under `/memories/user/` across threads. With [identity](/langsmith/managed-deep-agents-identity), memory is remounted per actor or tenant so callers cannot see each other's private state.
...
The agent never sees sibling actor or tenant trees. Isolation is the remount, not a multi-user listing under `/memories`.
...
| `none` | No `/memories/user` mount (and no hot injection) |
...
On first use of an actor or tenant slice, the runtime seeds `/memories/user/AGENTS.md` from the agent seed (or a default template) so hot memory instructions are present on turn one.

## Org memory (read-only)

Optional org-wide facts live under Context Hub `org-memory/` and mount at `/memories/org`. Agents may **read** org memory; writes under `/memories/org/**` are denied. Humans or org tooling update that tree—not the agent.
...
Actor-scoped local runs remount `memories/<actorId>/` as `/memories/user` the same way as deploy. After you change the SDK package used by the build, rebuild so `.mda/build/__runtime__` picks up the new runtime (vendored at compile time).

## Disable managed memory

Set `disableMemory` / `disable_memory` on the agent definition to skip hot injection and the `/memories/user` memory wiring. Identity `scoping.memory: "none"` also disables the mount.
