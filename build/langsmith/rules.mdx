...
1. Select an action to trigger when the rule is applied. There are four actions you can take with an automation rule:
    ...
    Other rule actions can also extend data retention on matching traces when their retention extension setting is enabled through the auto-upgrade mechanism described in the data retention docs, but this rule takes no additional action.
