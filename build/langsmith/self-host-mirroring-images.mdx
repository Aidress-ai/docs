...
The `--include-sandboxes` flag mirrors the LangSmith-owned sandbox runtime image. If your cluster cannot pull public images at all, also mirror the JuiceFS CSI images used by the sandbox storage driver:
...
images:
  ...
  juicefsCSINodeDriverRegistrarImage:
    ...
    tag: "v2.9.0"
  juicefsCSILivenessProbeImage:
    repository: "(your-registry)/sig-storage/livenessprobe"
    tag: "v2.12.0"
```

The JuiceFS CSI driver may also create JuiceFS mount pods at runtime. If those pods cannot pull the default public mount image, mirror a JuiceFS mount image and set both `images.juicefsMountImage.repository` and `images.juicefsMountImage.tag`.
