...
The complete list of scenarios in which a trace can upgrade:

* **Feedback** is added to any run on the trace (or any trace in the thread) through an API or SDK call that uses the default retention behavior. The LangSmith UI sends feedback and notes without forcing a retention upgrade.
* An **online evaluator** scores the trace and its retention setting is enabled. Project evaluators can opt out of this upgrade; thread evaluators and transient queue-population backfill rules do not force an upgrade by default.
* An **[annotation queue](/langsmith/annotation-queues#assign-runs-to-a-single-run-queue)** receives a run from a rule or API call that explicitly opts in to extending retention. Manually adding runs to a queue, including from experiment comparison views, does not upgrade retention by default.
* An **[automation rule](/langsmith/rules#create-a-rule)** with retention extension enabled matches any run within a trace.
...
* **Annotation Queues, Run Rules, and Feedback**: Traces that use these features may be [auto-upgraded](#data-retention-auto-upgrades) depending on the retention setting for the action that touched the trace.
...
2. add feedback when the feedback request would extend retention
3. add runs to annotation queues when the add operation would extend retention

Actions that do not change trace retention are not counted against the extended-retention trace limit.
