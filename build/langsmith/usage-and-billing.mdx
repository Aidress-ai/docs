...
A base-tier trace upgrades to extended retention in the following scenarios:

* **Feedback** is added programmatically to any run on the trace (or any trace in the thread) [via the SDK](/langsmith/attach-user-feedback). Feedback and reviewer notes added from the LangSmith UI do not upgrade retention.
* An **[online evaluator](/langsmith/online-evaluations-llm-as-judge)** scores the trace, unless you opt out of extending retention in the evaluator settings. For more information, see [Manage evaluator trace retention](/langsmith/evaluators#manage-evaluator-trace-retention).
* An **[annotation queue](/langsmith/annotation-queues#assign-runs-to-a-single-run-queue)** receives a run from the trace, unless the request sets `extend_trace_retention=false`.
* An **[automation rule](/langsmith/rules#create-a-rule)** matches a run within the trace and its action is configured to extend retention.
...
* **Annotation queues, automation rules, and feedback**: These features can [auto-upgrade traces](#data-retention-auto-upgrades), depending on how you configure or invoke them.
