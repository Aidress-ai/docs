...
* **Feedback** is added to any run on the trace (or any trace in the thread) through an API or SDK call that explicitly passes `extend_trace_retention=true` (`extendTraceRetention: true` in TypeScript). The LangSmith UI sends feedback and notes without forcing a retention upgrade.
* An **online evaluator** scores the trace and its retention setting is enabled. Trace and thread online evaluators can opt out of this upgrade.
...
* **Uses extended retention**: Experiment runs use extended retention by default.
