...
* An **online evaluator** scores the trace and its retention setting is enabled. Project evaluators can opt out of this upgrade; thread evaluators do not force an upgrade by default.
* An **[automation rule](/langsmith/rules#create-a-rule)** with retention extension enabled matches any run within a trace.

Manually adding runs to an [annotation queue](/langsmith/annotation-queues#assign-runs-to-a-single-run-queue), including from experiment comparison views, does not upgrade retention by default.
...
* **Run Rules and Feedback**: Traces that use these features may be [auto-upgraded](#data-retention-auto-upgrades) depending on the retention setting for the action that touched the trace.
* **Annotation Queues**: Manually adding runs to annotation queues does not change trace retention by default.
...
2. log feedback through an SDK/API request that extends trace retention
