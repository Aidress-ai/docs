...
`--yolo` takes priority over `-y`/`--auto-approve`, which takes priority over `[startup].mode`. The removed `dangerously-auto` startup value is no longer recognized and falls back to Manual.
...
<Accordion title="Advanced details" icon="settings">

### MCP read-only annotations

MCP servers can attach standard `ToolAnnotations` when advertising a tool. Deep Agents Code lets a tool bypass classifier review only when all of the following are true:
...
- [Remote sandboxes](/oss/python/deepagents/code/remote-sandboxes)
