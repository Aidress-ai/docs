...
Project subagents override user subagents with the same name (see [precedence rules](/oss/python/deepagents/code/configuration#subagents)).
...
`dcode` ships with the code interpreter enabled, so [dynamic subagents](/oss/python/deepagents/dynamic-subagents) work out of the box — no extra wiring required.
