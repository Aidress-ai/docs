...
<Note>Available for LangSmith Cloud US.</Note>
...
- **Tracing projects**: add one or multiple as needed per chart. Metrics are computed by pooling runs across every selected project into a single set, not shown per project. To break out results per project, use [Group by](#split-the-data).
...
Create multiple series on the same chart in one of two ways:
...
- **Chart filter**: applies to every series on the chart. Configured once at the top of the chart form.
- **Series filter**: applies only to the metric it is attached to. Configured inline with each series (used for the numerator and denominator of ratio metrics, or when comparing e.g. errors vs. successes across two series).

Tracing project sources support three filter types:

- **Run filter**: filters at the run level.
- **Trace filter**: filters at the trace (root-run) level.
- **Tree filter**: includes the entire trace tree if any run in it matches.

Dataset sources do not expose run/trace/tree filters. Data is scoped by the selected dataset. For filter syntax, see [filtering traces in application](/langsmith/filter-traces-in-application).
...
<Note>Available for LangSmith self-hosted deployments and LangSmith Cloud EU/APAC customers.</Note>
